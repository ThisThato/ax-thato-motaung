import type { BlogCard, BlogDetail, CommentItem, CodeSnippet, ContentBlock } from "../types";
...
const blockId = () => `seed-${Math.random().toString(36).slice(2, 10)}`;
...
const makeBlocks = (content: string, images: string[], codeSnippets: CodeSnippet[]): ContentBlock[] => {
    const paragraphBlocks = content
        .split("\n\n")
        .map((item) => item.trim())
        .filter(Boolean)
        .map<ContentBlock>((text) => ({ id: blockId(), type: "paragraph", text }));
...
export const seedArticles: MockArticle[] = [
    {
        ...
        contentBlocks: makeBlocks(
            "SOLID is a set of design principles that helps make software easier to maintain and evolve.\n\n- Single Responsibility Principle\n- Open/Closed Principle\n- Liskov Substitution Principle\n- Interface Segregation Principle\n- Dependency Inversion Principle\n\nIn C#, these principles usually become visible in how we shape interfaces, services, and dependency injection boundaries.",
            [
    ...
    },
    {
        ...
        contentBlocks: makeBlocks(
            "Cloud deployment succeeds when you design for observability, failure, and repeatability.\n\nStart with health checks, centralized logs, config separation, and CI/CD pipelines.\n\nThen validate scaling and graceful shutdown behavior before production rollout.",
            [
    ...
    },
    {
        ...
        contentBlocks: makeBlocks(
            "Optimize where users feel latency first: route transitions, large lists, and expensive client transforms.\n\nMeasure before and after each change using browser profiling.",
            [
